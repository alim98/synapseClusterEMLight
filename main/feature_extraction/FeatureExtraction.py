...
class FeatureExtraction:
    ...
    def extract_features(self, seg_type=None, alpha=None, extraction_method=None, layer_num=None):
        ...
        preprocessing = getattr(self.config, 'preprocessing', 'normal')
        use_intelligent_cropping = preprocessing == 'intelligent_cropping'
        presynapse_weight = getattr(self.config, 'preprocessing_weights', 0.5)
        
        # Update processor settings if using intelligent cropping
        if hasattr(self.processor, 'use_intelligent_cropping'):
            self.processor.use_intelligent_cropping = use_intelligent_cropping
        if hasattr(self.processor, 'presynapse_weight'):
            self.processor.presynapse_weight = presynapse_weight
        ...
        dir_components = [
            ...
            f"seg{seg_type}",
            f"alpha{alpha}",
            f"intelligent_crop_w{presynapse_weight}" if use_intelligent_cropping else None
        ]
        ...
        if use_intelligent_cropping:
            print(f"Using intelligent cropping with weight {presynapse_weight}")
